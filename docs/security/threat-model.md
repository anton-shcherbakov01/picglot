...
Scope: the LingoImage AI web app, public API, workers and stored data.
...
*Test:* `test_a_user_cannot_read_another_users_project`.
...
enabled. *Tests:* `test_magic_bytes_beat_the_declared_extension`,
...
stream that inflates past 40 MB. *Test:* `test_oversized_images_are_rejected_before_decoding`.
...
are removed. *Test:* `test_pdf_javascript_is_stripped_on_upload`.
...
never used as storage keys. *Test:* `test_filenames_are_sanitised`.
...
*Tests:* `test_forged_and_replayed_webhooks_are_rejected`.
...
Redirects are not followed. *Test:* `test_webhook_urls_pointing_at_internal_hosts_are_refused`.
...
the ledger sum. *Tests:* `test_charging_the_same_job_twice_debits_once`,
