...
Scope: what personal and document data PicGlot collects, where it goes,
...
- **The LLM provider** (handwriting, hard OCR, structured extraction):
  document content is passed as delimited, untrusted _data_ — the system
...
production instance is `python -m picglot.cli config` run against its own
