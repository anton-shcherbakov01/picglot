...
http_requests_total = Counter(
    "lingoimage_http_requests_total",
    "HTTP requests handled",
    ["method", "route", "status"],
)
http_request_duration = Histogram(
    "lingoimage_http_request_duration_seconds",
    "HTTP request duration",
    ["method", "route"],
    buckets=(0.01, 0.05, 0.1, 0.25, 0.5, 1, 2.5, 5, 10, 30),
)
jobs_total = Counter(
    "lingoimage_jobs_total",
    "Jobs by type and terminal status",
    ["type", "status"],
)
job_duration = Histogram(
    "lingoimage_job_duration_seconds",
    "End to end job duration",
    ["type"],
    buckets=(1, 2.5, 5, 10, 20, 40, 80, 160, 320, 640),
)
job_stage_duration = Histogram(
    "lingoimage_job_stage_duration_seconds",
    "Duration of a single pipeline stage",
    ["stage"],
    buckets=(0.1, 0.5, 1, 2.5, 5, 10, 30, 60, 120),
)
pages_processed_total = Counter(
    "lingoimage_pages_processed_total",
    "Pages pushed through the pipeline",
    ["tool"],
)
provider_calls_total = Counter(
    "lingoimage_provider_calls_total",
    "Provider invocations",
    ["kind", "provider", "status"],
)
provider_latency = Histogram(
    "lingoimage_provider_latency_seconds",
    "Provider call latency",
    ["kind", "provider"],
    buckets=(0.1, 0.25, 0.5, 1, 2, 5, 10, 30, 60, 120),
)
provider_cost_usd = Counter(
    "lingoimage_provider_cost_usd_total",
    "Estimated provider spend",
    ["kind", "provider"],
)
provider_daily_cost_usd = Gauge(
    "lingoimage_provider_daily_cost_usd",
    "Estimated provider spend today",
    ["kind", "provider"],
)
provider_daily_cost_limit_usd = Gauge(
    "lingoimage_provider_daily_cost_limit_usd",
    "Configured daily spend cap",
    ["kind", "provider"],
)
provider_circuit_open = Gauge(
    "lingoimage_provider_circuit_open",
    "1 when the circuit breaker for a provider is open",
    ["kind", "provider"],
)
queue_depth = Gauge(
    "lingoimage_queue_depth",
    "Queued jobs awaiting a worker",
    ["queue"],
)
worker_heartbeat_age = Gauge(
    "lingoimage_worker_heartbeat_age_seconds",
    "Seconds since the most recent worker heartbeat",
)
credits_consumed_total = Counter(
    "lingoimage_credits_consumed_total",
    "Credits debited",
    ["reason"],
)
credits_refunded_total = Counter(
    "lingoimage_credits_refunded_total",
    "Credits refunded",
    ["reason"],
)
translation_characters_total = Counter(
    "lingoimage_translation_characters_total",
    "Characters submitted for translation",
    ["provider"],
)
webhook_delivery_failures_total = Counter(
    "lingoimage_webhook_delivery_failures_total",
    "Outbound webhook deliveries that failed",
    ["event"],
)
payment_webhook_failures_total = Counter(
    "lingoimage_payment_webhook_failures_total",
    "Inbound payment webhooks rejected or failed",
    ["provider", "reason"],
)
rate_limit_hits_total = Counter(
    "lingoimage_rate_limit_hits_total",
    "Requests rejected by rate limiting",
    ["scope"],
)
db_up = Gauge("lingoimage_db_up", "1 when the database responds to a health probe")
storage_used_bytes = Gauge("lingoimage_storage_used_bytes", "Bytes stored in object storage")
storage_capacity_bytes = Gauge("lingoimage_storage_capacity_bytes", "Configured storage capacity")
cache_hits_total = Counter("lingoimage_cache_hits_total", "Cache hits", ["cache"])
cache_misses_total = Counter("lingoimage_cache_misses_total", "Cache misses", ["cache"])
