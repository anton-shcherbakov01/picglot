"""Operational CLI: `python -m lingoimage.cli <command>`."""
...
from lingoimage import __version__
from lingoimage.core.config import ConfigurationError, settings
from lingoimage.core.logging import configure_logging, get_logger
...
def cmd_health(args: argparse.Namespace) -> int:
    ...
    from lingoimage.core.redis import redis_healthy
    from lingoimage.db.session import database_healthy
    ...
    from lingoimage.providers.llm import get_llm
    from lingoimage.services.storage import get_storage
    ...
    if not any_ocr:
        print(_fail("no OCR provider available", "install extras: pip install 'lingoimage[ocr]'"))
        failures += 1
...
def cmd_wait_for_db(args: argparse.Namespace) -> int:
    from lingoimage.db.session import database_healthy, dispose_engine
...
def cmd_seed(args: argparse.Namespace) -> int:
    from lingoimage.db.seed import run_seed
...
def cmd_create_admin(args: argparse.Namespace) -> int:
    ...
    from lingoimage.domain.enums import AdminRole, UserStatus
...
def cmd_install_language_pack(args: argparse.Namespace) -> int:
    from lingoimage.providers.translation.local import install_language_pack
...
def cmd_config(args: argparse.Namespace) -> int:
    ...
    from lingoimage.core.security import mask_secret
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="lingoimage", description=f"{settings.brand_name} CLI")
