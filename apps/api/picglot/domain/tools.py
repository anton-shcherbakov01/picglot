...
from picglot.domain.enums import ExportFormat, JobType, ToolType
...
#: Locale-specific slugs for tools whose search demand is in the local language.
#:
#: A locale listed here uses its own slug *instead of* the English one — it is a
#: replacement, not an addition, so the two never compete as duplicate content.
#: `/{locale}/{english-slug}` keeps working and redirects to the localised path.
LOCALIZED_SLUGS: dict[ToolType, dict[str, str]] = {
    ToolType.TRANSLATE_PHOTO: {"ru": "perevod-po-foto"},
    ToolType.IMAGE_TO_TEXT: {"ru": "tekst-s-kartinki"},
    ToolType.JPG_TO_WORD: {"ru": "foto-v-word"},
    ToolType.IMAGE_TO_EXCEL: {"ru": "foto-v-excel"},
    ToolType.PDF_TRANSLATOR: {"ru": "perevod-pdf"},
}


def slug_for(tool: ToolSpec, locale: str) -> str:
    """The slug this tool is published under in `locale`."""
    return LOCALIZED_SLUGS.get(tool.type, {}).get(locale, tool.slug)


def localized_slugs(tool: ToolSpec) -> dict[str, str]:
    """Locale -> slug, for every locale that overrides the English slug."""
    return dict(LOCALIZED_SLUGS.get(tool.type, {}))
...
#: A localised slug must not collide with any English tool or format-page slug,
#: or one path would have to serve two different pages in the same locale.
_LOCALIZED = {slug for by_locale in LOCALIZED_SLUGS.values() for slug in by_locale.values()}
assert not (_LOCALIZED & set(BY_SLUG)), "a localised slug collides with a tool slug"
assert not (_LOCALIZED & {slug for slug, *_ in FORMAT_PAGES}), (
    "a localised slug collides with a format page slug"
)
assert len(_LOCALIZED) == sum(len(by_locale) for by_locale in LOCALIZED_SLUGS.values()), (
    "two tools claim the same localised slug"
)
