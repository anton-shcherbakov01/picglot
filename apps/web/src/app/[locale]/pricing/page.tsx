...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: { canonical: absoluteUrl(localePath(locale, 'pricing')), languages },
  };
}
...
const FEATURE_LABELS: Record<string, { en: string; ru: string }> = {
  ...
  advanced_inpaint: { en: 'Advanced background repair', ru: 'Улучшенное восстановление фона' },
  glossary: { en: 'Glossaries', ru: 'Глоссарии' },
  translation_memory: { en: 'Translation memory', ru: 'Память переводов' },
  email_notifications: { en: 'Email notifications', ru: 'Уведомления по email' },
  ...
  central_billing: { en: 'Centralised billing', ru: 'Централизованный биллинг' },
  priority_support: { en: 'Priority support', ru: 'Приоритетная поддержка' },
  custom_retention: { en: 'Custom retention period', ru: 'Настраиваемый срок хранения' },
  local_only_processing: { en: 'Local-only processing mode', ru: 'Режим локальной обработки' },
  ...
  return (
    <div className="container-page py-12 sm:py-16">
      <div className="mx-auto max-w-2xl text-center">
        <h1 className="text-3xl font-bold sm:text-4xl">{messages.pricing.title}</h1>
          ...
          return (
            ...
            >
              ...
              <p className="mt-2 text-sm text-muted">
                {formatNumber(plan.monthly_credits, typed)} {messages.dashboard.credits.replace('{count} ', '')}
              </p>
              ...
              <Link
                href={localePath(typed, plan.code === 'free' ? 'auth/sign-up' : 'app/billing')}
                className={`mt-6 ${highlight ? 'btn-primary' : 'btn-secondary'}`}
              >
                {plan.code === 'free' ? messages.pricing.getStarted : messages.pricing.upgrade}
              </Link>
            </section>
          );
        })}
      ...
      <section className="mt-12" aria-labelledby="credits-heading">
              ...
              <tbody>
                {config.credit_rules.map((rule, index) => (
                  <tr key={index} className="border-t border-border">
                    <td className="px-4 py-3">{String(rule.key).replace(/_/g, ' ')}</td>
        ...
        <p className="mt-3 text-sm text-muted">
          Re-downloading an existing result is free. A job that fails for a technical reason is
          refunded automatically, and cancelling before processing starts costs nothing.
        </p>
      </section>
    </div>
  );
