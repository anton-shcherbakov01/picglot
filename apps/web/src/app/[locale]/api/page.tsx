...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: absoluteUrl(localePath(locale, "api")),
      languages,
    },
  };
}
  ...
  return (
    <div className="container-page max-w-4xl py-12">
      <h1 className="text-3xl font-bold">
        {ru ? "API для разработчиков" : "Developer API"}
      </h1>
      ...
      <section className="mt-10">
        <h2 className="text-xl font-semibold">
          {ru ? "Идемпотентность" : "Idempotency"}
        </h2>
        <p className="mt-2 text-muted">
          {ru
      ...
      <section className="mt-10">
        <h2 className="text-xl font-semibold">
          {ru ? "Статусы задания" : "Job statuses"}
        </h2>
      ...
      <section className="mt-10">
        ...
        <pre className="scroll-x mt-3 rounded-card border border-border bg-raised p-4 text-xs">
          <code>{`X-PicGlot-Signature: t=1735689600,v1=<hex>
X-PicGlot-Event: job.completed
X-PicGlot-Event-Id: evt_…
X-PicGlot-Delivery: dlv_…`}</code>
        </pre>
      </section>
